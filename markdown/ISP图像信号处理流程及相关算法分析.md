...
彩色图像的每一个像素点都需要红、绿、蓝三个色彩分量的信息，而由Bayer阵列所产生的RAW图中的每一个像素点只有红或绿或蓝一个色彩分量的信息，因此ISP必须估算出每个像素处缺失的另外两个颜色值，这个过程叫做色彩滤波阵列插值（CFAI），也叫去马赛克（Demosaicing）。
...
Y_{out}=Y_{in}^\gamma
$$
不同伽马值下校正效果如下图，(a)图中的伽马值r=0.5，(b)图中r=1，(c)图中r=2。可以观察到，当伽马值小于1时，图像的整体亮度值得到提升，同时低灰度处的对比度得到增加，更利于分辩低灰度值时的图像细节。在伽马校正过程中，我们一般让伽马值r<1，从而让暗部细节更明显，通过调节伽马值，我们可以得到具有较好对比度和动态范围的图像。
...
CSC即色彩空间转换，通过一些线性变化，将原本图像的颜色空间转换到其他的颜色空间，常见的有RGB2YUV，RGB2SV等等，但是通常在ISP的Pipeline中用到的CSC转换只有RGB2YUV，YUV色彩空间中，“Y”表示明亮度（Luminance），“U”和“V”则是色调（Hue）、色彩饱和度（saturation）。ISP要实现RGB2YUV转换就必定会有一个转换公式，因为转换过程中采用的标准不同，所有转换公式也有所区别。由于CSC算法固定，所以它一般固化在ISP芯片中。

RGB颜色空间到YUV颜色空间的转换标准如下：

**1. BT.601标准[1]——标清数字电视（SDTV)**

![](D:\notes\markdown\ISP图像信号处理流程及相关算法分析.assets\YUV_SDTV.png)

**2. BT.709标准[2]——高清数字电视（HDTV)**

![](D:\notes\markdown\ISP图像信号处理流程及相关算法分析.assets\YUV_HDTV.png)

**3. BT.2020标准[3]——超高清数字电视（UHDTV)**

![](D:\notes\markdown\ISP图像信号处理流程及相关算法分析.assets\YUV_UHDTV.png)
...
1. 双边滤波
   ...
   $$
   G_{\sigma_{s}}(\|p-q\|)=exp(-\frac{(i-k)^2+(j-l)^2}{2\sigma_{s}^2}) \\
   G_{\sigma_{r}}(|I_p-I_q|)=exp(-\frac{\|f(i,j)-f(k,l)\|^2}{2\sigma_{r}^2})
   $$
...
2. NLM（non-local mean）
   ...
   NLM是利用整个图像的信息来进行降噪滤波处理。如下图所示，对于p点而言，q1和q2点所在的邻域和p所在的邻域更相似，那么就给q1和q2较大的权重，而q3邻域和p邻域差别较大，就赋予一个较小的权重。具体权重的赋予方式其实也是高斯分布的一种方式，只不过e的对数是通过邻域简单欧拉距离来计算。
   ...
   ![](D:\notes\markdown\ISP图像信号处理流程及相关算法分析.assets\NLM_1.png)

   ![](D:\notes\markdown\ISP图像信号处理流程及相关算法分析.assets\NLM_2.png)
...
人眼之所以能感受到自然界的颜色是因为人眼的视锥细胞在起作用，人眼主要通过三种视锥细胞感受三种不同波长的光。430nm波长的光被视为蓝色，主要激活S型锥形细胞；大多数波长以550nm为中心的光被视为绿色，主要刺激M型锥形细胞；600nm波长或更长波长的光被视为红色，主要刺激L型锥形细胞。如图所示是人眼感受不同波长的光的光谱灵敏度反应曲线：
